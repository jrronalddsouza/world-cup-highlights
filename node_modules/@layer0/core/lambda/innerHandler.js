...
async function startApp(port) {
    ...
    if (fs_extra_1.existsSync(modulePath)) {
        ...
        // use default JS backend port when no port is provided
        const imageOptimizerPort = process.env.LAYER0_IMAGE_OPTIMIZER_PORT;
        // check if provided env variable port can be parsed
        if (imageOptimizerPort && !/^\d+$/.test(imageOptimizerPort)) {
            console.error('Error: LAYER0_IMAGE_OPTIMIZER_PORT is not valid number');
            process.exit(1);
        }
        ...
        router.setBackend(constants_1.BACKENDS.imageOptimizer, {
            ...
            port: imageOptimizerPort ? parseInt(imageOptimizerPort) : undefined,
        });
    }
}
/**
