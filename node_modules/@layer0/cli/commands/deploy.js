...
const getCoreLayer0Version = require('../utils/getCoreLayer0Version');

const buildProject = require('../utils/build');

const {
  readLayer0Version
} = require('../utils/layer0');
...
const getLayer0Config = require('../utils/getLayer0Config');

const Validator = require('../utils/validator');

exports.command = 'deploy [team]';
exports.describe = 'Deploys your project to Layer0';
...
class DeployCommand {
  constructor(yargs) {
    _defineProperty(this, "_wrappedRun", async () => {
      ...
      const packageJson = Layer0PackageJson.loadPackageJson(givenPath);
      const coreLayer0Version = getCoreLayer0Version();

      if (isEmpty(coreLayer0Version)) {
        throw new DeployError('@layer0/core module has not been installed, please run "layer0 init".');
      }
      ...
      const siteSlug = site || config.name || packageJson.name;
      ...
      logger.info([`> Team: ${teamSlug || 'Private space'}`, `> Site: ${siteSlug}`, branchName && `> branch: ${branchName}`, `> Environment: ${environment || 'default'}`, `> Layer0 version: ${coreLayer0Version}`, commitUrl && `> Commit URL: ${commitUrl}`].filter(Boolean) // Removes falsy values
      .join(EOL));
      const {
        build: {
          id: buildId,
        ...
        teamSlug,
        environment,
        layer0Version: coreLayer0Version,
        branch: branchName,
      ...
      const zippingStep = async () => {
        // Check that Layer0 versions match unless we are overriding the versions for platform development.
        if (isEmpty(process.env.LAYER0_VERSION_OVERRIDE)) {
          const builtLayer0Version = await readLayer0Version(absolutePath);

          if (coreLayer0Version !== builtLayer0Version) {
            throw new DeployError(`Please rebuild with Layer0 v${coreLayer0Version}`);
          }
        } // Puts projectRoot/.layer0 folder into zip
    ...
    _defineProperty(this, "_streamLogsUntilStatus", async statuses => {
      ...
      while (!build || !statuses.includes(build.status)) {
        ...
        if (build.status === 'failed') {
          throw new Layer0BuildError(build);
        }
      }
  ...
  _processSignalHandler(signal) {
    const error = new Layer0CLIError(`${signal} raised`);
  ...
  async _failLayer0Build(e) {
    // Layer0BuildError are already originating from a failed build on Layer0 so we don't need
    // to push that information back to LeDeployer. It would also result in duplicated logs
    if (e instanceof Layer0BuildError) {
      return;
    }
...
/**
 * Returns the contents of layer0.config.js, or, if not present, exits with
 * an error message.
 * @returns
 */
...
function getConfig(logger) {
  const config = getLayer0Config();

  if (!config) {
    logger.error(`Error: layer0.config.js not found. Please run "0 init" to initialize your Layer0 project.\n`);
    process.exit(1);
  }
